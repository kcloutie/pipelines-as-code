...
func TestGiteaParamsChangedFilesCEL(t *testing.T) {
	...
	assert.Equal(t, len(repo.Status), 1, repo.Status)
	// check the output logs if the CEL body headers has expanded  properly
	output := `all_changed_files: .tekton/pullrequest.yaml,.tekton/push.yaml,deleted.txt,modified.txt,renamed.txt
added_files: .tekton/pullrequest.yaml,.tekton/push.yaml,deleted.txt,modified.txt,renamed.txt
deleted_files: 
modified_files: 
renamed_files:`
	err = twait.RegexpMatchingInPodLog(context.Background(),
...
renamed_files:`
	...
	// check the output logs if the CEL body headers has expanded  properly
	output = `all_changed_files: deleted.txt,hasbeenrenamed.txt,modified.txt
added_files: 
deleted_files: deleted.txt
modified_files: modified.txt
renamed_files: hasbeenrenamed.txt`
	err = twait.RegexpMatchingInPodLog(context.Background(),
		topts.ParamsRun,
		topts.TargetNS,
		fmt.Sprintf("tekton.dev/pipelineRun=%s,tekton.dev/pipelineTask=changed-files-pullrequest-params",
	...
	// check the output of the last status PipelineRun which should be a
	// push matching the expanded CEL body and headers values
	output = `all_changed_files: deleted.txt,hasbeenrenamed.txt,modified.txt,renamed.txt
added_files: hasbeenrenamed.txt
deleted_files: deleted.txt,renamed.txt
modified_files: modified.txt
renamed_files:`
	err = twait.RegexpMatchingInPodLog(context.Background(),
		topts.ParamsRun,
